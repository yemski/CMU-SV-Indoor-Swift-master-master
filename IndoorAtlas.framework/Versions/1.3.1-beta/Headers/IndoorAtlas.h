 ...
 *     - *format*: Image file data format (jpeg, png, svg). If an error occured, this parameter is nil.
 *     - *error*: If an error occured, this object describes the error. If the operation completed successfully, this value is nil.
 *
 * @return IndoorAtlasFetch instance for the request.
 */
+ (id<IndoorAtlasFetch>)fetchFloorplanImage:(IndoorAtlasFloorplan*)floorplan
                                 completion:(void(^)(NSData *data, NSString *format, NSError *error))completionBlock;
...
/**
 * @name Get calibrator singleton
 */

/**
 * Returns calibrator singleton.
 *
 * @return <IndoorAtlasCalibrator> instance.
 */
+ (IndoorAtlasCalibrator*)calibrator;
